      ...
      >
        ...
        <div class="mt-2 w-3/4 rounded-tl-lg bg-white shadow-md p-3 mb-6 text-xl">
          ...
          >
            ...
            <div class="flex gap-3 justify-between mt-1">
              <v-text-field
                v-model="total_price_with_copupon"
                label="الإجمالي بعد الكوبون"
                placeholder="الإجمالي بعد الكوبون"
                type="number"
                variant="outlined" 
                :error-messages="errors.total_price_with_copupon"
                @input="convertToNumber"
              />
            </div>
            ...
            <div class="flex gap-3 justify-between mt-1">
              <v-text-field
                ...
                type="number"
                variant="outlined" 
                :error-messages="errors.debt_ratio"
                @input="convertToNumber"
              />
            </div>
          </div>
...
watchEffect(() => {
  if ( orderDetails.value) {
    ...
    orderDetails.value.products.forEach((product) => {
      quantityInputs.value.push(product.minimum_quantity)
...
const total = computed(() => {
    return orderDetails.value!.products.reduce((acc, procut) => acc + procut.price, 0);
});
